...
(defn combine-results [result-files]
  (let [forms (mapv (comp edn/read-string slurp) result-files)
        _ (assert (seq forms))
        ...
        {:keys [partition-target-minutes max-partitions]} (get-in base [:kaocha/cli-options])]
    (cond-> base
      partition-target-minutes (assoc :suggested-partitions
                                      (let [max-partitions (or max-partitions 10)
                                            _ (assert (pos? max-partitions))
                                            partition-target-ns (*' 60 60 1e6 partition-target-minutes)
                                        ...
                                        (-> (/ total-duration-ns partition-target-ns)
                                            Math/ceil
                                            (max max-partitions)
                                            (min 1)))))))
